export default {
    ...
    validatePhone(phoneValue) {
        if (phoneValue === undefined || phoneValue === null || phoneValue.length === 0) {
            return false;
        }
        phoneValue = this.valueTrim(phoneValue);
        var reg = /^[1][0-9]{10}$/;
    ...
    validateEmail(emailValue) {
        if (emailValue === undefined || emailValue === null || emailValue.length === 0) {
            return false;
        }
        var reg = /^\w+([-+.]\w+)*@\w+([-.]\w+)*\.\w+([-.]\w+)*$/;
     ...
     * @returns 匹配返回true 不匹配返回false
     */
    isMyFloat(floatValue) {
        if (floatValue == '') {
            return false;
        }
        floatValue = this.valueTrim(floatValue);
        var reg = /^(\d+)(\.\d+)$/;
        if (this.isNumber(floatValue)) {
            return true;
        }
    ...
    valiDateTel(telValue) {
        ...
        telValue = this.valueTrim(telValue);
     ...
     * @returns 匹配返回true 不匹配返回false
     */
    IdCardValidate(idCardValue) {
        let Wi = [7, 9, 10, 5, 8, 4, 2, 1, 6, 3, 7, 9, 10, 5, 8, 4, 2, 1];    // 加权因子
        // 身份证验证位值.10代表X
        //去掉字符串头尾空格
        idCardValue = this.valueTrim(idCardValue.replace(/ /g, ""));
        if (idCardValue.length == 15) {
            //进行15位身份证的验证
            return this.isValidityBrithBy15IdCard(idCardValue);
        }
        else if (idCardValue.length == 18) {
            // 得到身份证数组
            var a_idCard = idCardValue.split("");
            //进行18位身份证的基本验证和第18位的验证
            if (this.isValidityBrithBy18IdCard(idCardValue) && this.isTrueValidateCodeBy18IdCard(a_idCard)) {
                return true;
            }
            else {
                return false;
